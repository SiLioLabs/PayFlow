...
export default function SubscriptionHealthWidget({ userKey }: SubscriptionHealthWidgetProps) {
    ...
    return (
      <div className="card" style={{ padding: "var(--space-3)", marginBottom: "var(--space-4)" }}>
        <div style={{ display: "flex", gap: "var(--space-2)" }}>
          <div className="skeleton-block" style={{ width: "20px", height: "20px", borderRadius: "50%" }}></div>
  ...
  return (
    <div className={`card ${isHealthy ? 'status-active' : 'status-danger'}`} style={{ padding: "var(--space-3)", marginBottom: "var(--space-4)", display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
      ...
      {!isHealthy && (
        <ul style={{ margin: 0, paddingLeft: "var(--space-5)", fontSize: "0.875rem", color: "var(--color-text-muted)" }}>
      ...
      {health.trial_active && isHealthy && (
        <p style={{ margin: 0, fontSize: "0.875rem", color: "var(--color-text-muted)" }}>Currently in trial period.</p>
      )}
    </div>
  );
}
